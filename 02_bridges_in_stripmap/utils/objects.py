...
class Cave(VGroup):
    def __init__(self, radius, color=GREY, fill_opacity=0.5, num_points=20, **kwargs):
        ...
        for i in range(num_points):
            ...
            r = radius * (0.8 + 0.4 * np.random.rand())  # vary radius for organic shape
            x = r * np.cos(angle)
            y = r * np.sin(angle)
...
class Dune(VGroup):
    ...
    def dune_curve(amplitude, wavelength, shift, height_shift, extent, color=BLACK):
        return ParametricFunction(
            lambda t: np.array(
                [t, amplitude * np.sin((t + shift) / wavelength) + height_shift, 0]
            ),
            t_range=extent,
            color=color,
        ).set_stroke(width=6)
    ...
    def fill_with_dots(
        ...
        for _ in range(n_dots):
            ...
            d = Dot(point=[x + jitter_x, y + jitter_y, 0], radius=radius, color=color)
            dots.add(d)
        return dots
...
class Sat(VGroup):
    def __init__(self, **kwargs):
        ...
        top_rect.next_to(bottom_rect, UP, buff=0).align_to(bottom_rect, RIGHT + LEFT)
    ...
    def send_beam(
        ...
        animations = [t_tracker.animate.set_value(3), pos_tracker.animate.set_value(1)]
...
class Crosshair(VGroup):
    def __init__(self, **kwargs):
        ...
        horizontal2 = Line(0.6 * RIGHT, 1.8 * RIGHT, color=BLACK, stroke_width=8)
...
class SecretDocument(VGroup):
    def __init__(self, **kwargs):
        ...
        self.lines = VGroup(
            *[
                Line(start=[-1.1, y, 0], end=[1.1, y, 0], stroke_width=3, color=GRAY_C)
